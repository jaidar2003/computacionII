...
import sys
import socket
from cli.utils.config import SERVER_HOST, SERVER_PORT
from cli.utils.session import load_session, check_auth
from cli.utils.connection import create_ssl_connection, send_command, upload_file as conn_upload_file, download_file as conn_download_file, receive_prompt, send_response
...
def list_files(silent=False):
    ...
    # Crear conexión SSL
    connection = create_ssl_connection(SERVER_HOST, SERVER_PORT)
    if not connection:
        if not silent:
